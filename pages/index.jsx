...
export default function Dashboard() {
  ...
  const chartData = (history.rows || []).map((r) => ({
    t: new Date(r.t).toLocaleTimeString("en-IN", {
      hour: "2-digit",
      minute: "2-digit",
    }),
    count: r.count,
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={`Live player stats for ${SERVER_NAME} FiveM server`}
        />
      </Head>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <button
            ...
            onMouseEnter={(e) => {
              ...
              e.target.style.background = "rgba(0,200,240,0.08)";
            }}
            onMouseLeave={(e) => {
          ...
          <StatCard
            ...
            sub={
              peakStats.trackingSince
                ? `since ${new Date(peakStats.trackingSince).toLocaleDateString()}`
                : "tracked"
            }
            accent="var(--accent2)"
            delay={0.25}
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {RANGES.map((r) => (
                  <button
                    ...
                    style={{
                      background:
                        range === r.hours ? "rgba(0,200,240,0.15)" : "none",
                      border: `1px solid ${range === r.hours ? "var(--accent)" : "var(--border)"}`,
                      color:
                        range === r.hours ? "var(--accent)" : "var(--muted)",
                      fontFamily: "var(--font-mono)",
                      fontSize: 10,
                      padding: "3px 10px",
                      borderRadius: 3,
            ...
            <div style={{ padding: "16px 8px 8px", height: 260 }}>
              {chartData.length === 0 ? (
                ...
                >
                  {loading
                    ? "loading..."
                    : "no data yet — cron will populate this"}
                </div>
              ) : (
